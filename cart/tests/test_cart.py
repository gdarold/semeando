...
from doacao.tests.factories import ProductFactory
...
def test_add_product_to_empty_cart(doacao, cart, session):
    cart.add(doacao)

    assert session[settings.CART_SESSION_ID] == {
        str(doacao.id): {"quantidade": 1, "peso": str(doacao.peso)}
    }
    assert session.modified


def test_add_product_to_empty_cart_quantity_gt_1(doacao, cart, session):
    cart.add(doacao, 2)

    assert session[settings.CART_SESSION_ID] == {
        str(doacao.id): {"quantidade": 2, "peso": str(doacao.peso)}
    }
    assert session.modified


def test_add_product_to_empty_cart_twice(doacao, cart, session):
    cart.add(doacao)
    session.modified = False
    ...
    assert session[settings.CART_SESSION_ID] == {
        str(doacao.id): {"quantidade": 3, "peso": str(doacao.peso)}
    }
    assert session.modified
...
def test_remove_product(doacao, cart, session):
    cart.add(doacao)
    session.modified = False
...
def test_get_total_price(cart):
    ...
    assert cart.get_total_peso() == total_price
